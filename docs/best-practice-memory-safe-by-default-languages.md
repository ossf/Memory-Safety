...
* Monitor occurrences of `sun.misc.Unsafe` or `jdk.internal.misc.Unsafe` in your code (or prevent them) using for example [checkstyle's IllegalImport rule configured to detect both sun._ and jdk.internal._](https://checkstyle.org/checks/imports/illegalimport.html).
